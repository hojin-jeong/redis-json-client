...
module.exports =
    class RedisJSON {
        ...
        async $_callCommand(command, args) {
            ...
            const r = await cmd.string.call(this.$redis, args).catch(err => err)
        ...
        async set(key, path, value, opts) {
            ...
            if(r instanceof Error) {
                ...
                if(opts.recursive) {
                    const recursiveErrs = [
                        'non-terminal path level',
                        'must be created at the root',
                        'at level 0 in path',
                        'objects must be created at the root',
                    ]
                    if(recursiveErrs.some(e => message.indexOf(e) !== -1)) {
