...
module.exports =
    class RedisJSON extends EventEmitter {
        ...
        mget(keys, path) {
            return new Promise((resolve, reject) => {
                path = this.$_pathMaker(path)
                const cmd = this.$_getInternalCommand('JSON.MGET')
                cmd.string.call(this.$redis, ...keys, path)
                    .then(result => {
                        const json = {}
                        for(let i=0; i<keys.length; i++) {
                            json[keys[i]] = JSON.parse(result[i])
                        }
                        resolve(json)
                    })
                    .catch(reject)
        ...
        /**
         * 해당 path의 저장된 숫자를 value만큼 증가시킨다.
         *
         * 시간 복잡도: O(1)
         * @param key
         * @param path
        ...
        inc(key, path, value) {
            path = this.$_pathMaker(path)
            const cmd = this.$_getInternalCommand('JSON.NUMINCRBY')
            return cmd.string.call(this.$redis, key, path, value)
        }

        /**
         * 해당 path의 저장된 숫자를 value만큼 곱한다.
         *
         * 시간 복잡도: O(1)
         * @param key
         * @param path
        ...
        mul(key, path, value) {
            path = this.$_pathMaker(path)
            const cmd = this.$_getInternalCommand('JSON.NUMMULTBY')
            return cmd.string.call(this.$redis, key, path, value)
        }
    }